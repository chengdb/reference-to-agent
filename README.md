...
- **配方**：左侧列表可增删；选中后编辑名称与步骤。步骤可增删、上下调整顺序，每步选类型后按字段填参数；快捷键类步骤支持录制，「激活应用 / 聚焦应用」可通过「选择应用…」从本机已安装应用（开始菜单快捷方式）中选取，并自动填入标题与 exe 路径。
...
| `activateApp` | `title`、`exe?` | 按标题模糊匹配激活窗口；未找到且给了 `exe` 时先启动该程序并等待窗口出现 |
